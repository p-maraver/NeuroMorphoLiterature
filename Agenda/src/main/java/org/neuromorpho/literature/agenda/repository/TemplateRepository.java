...
public class TemplateRepository {
    ...
    public TemplateRepository(
            ...
            @Value("${spring.data.mongodb.collection.template}") String COLLECTION_NAME) {
    ...
    public Email findOneByType(String type) {
        Bson match = Filters.eq("type", type);
        FindIterable<Email> resultList = collection.find(match);
        return resultList.into(new ArrayList<>()).get(0);
    }

    public void update(String id, Email template) {
        collection.replaceOne(Filters.eq("_id", new ObjectId(id)), template);
    }
